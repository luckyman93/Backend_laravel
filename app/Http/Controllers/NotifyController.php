...
class NotifyController extends Controller
{
    ...
    public function getNotify(Request $request){
        ...
        $arrMyfeedbacks = Feedback::where('user_id', '=', $user_id)->get();

        $arrMyFeedbackIds = array();
        foreach ($arrMyfeedbacks as $myfeedback) {
            array_push($arrMyFeedbackIds, $myfeedback->_id);
        }

        $feedbackcomments = FeedbackComment::whereIn('feedback_id', $arrMyFeedbackIds)->where('notify', '=', 0)->get();
        ...
        if(count($feedbackcomments) > 0){
            foreach ($feedbackcomments as $key => $feedbackComment) {
                ...
                $feedback = $feedbackComment->feedback()->first();
                $feedbackComment->feedback_title = $feedback->title;
            }
        }
